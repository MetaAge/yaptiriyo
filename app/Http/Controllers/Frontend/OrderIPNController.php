...
class OrderIPNController extends Controller
{
    ...
    public function iyzipay_ipn_for_order()
    {
        ...
        if (isset($payment_data['status']) && $payment_data['status'] === 'complete'){
            ...
            $user_id = session()->get('user_id') ?? ($order_details->user_id ?? null);
            $freelancer_id = session()->get('freelancer_id') ?? ($order_details->freelancer_id ?? null);
            $project_or_job = session()->get('project_or_job') ?? ($order_details->is_project_job ?? null);
            $proposal_id = session()->get('proposal_id_for_order') ?? ($order_details->identity ?? null);
    ...
    private function update_database($last_order_id,$transaction_id,$user_id,$freelancer_id,$project_or_job,$proposal_id)
    {
        ...
        if(moduleExists('CurrencySwitcher')){
            ...
            $transaction_amount = $order_info->transaction_amount / $conversion_rate;
        ...
        freelancer_notification($last_order_id, $freelancer_id,'Order','You have a new order');
        event(new ProjectEvent(__('You have a new order'), $freelancer_id));
