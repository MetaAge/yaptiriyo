    ...
    */
    ...
    public function userRegister(Request $request)
    {
        if ($request->isMethod('post')) {
            $request->validate([
                ...
                'confirm_password.required' => __('Please confirm your password'),
                'confirm_password.same' => __('Passwords do not match'),
                'terms_condition.required' => __('You must accept the terms and conditions'),
            ]);

            if (!empty(get_static_option('site_google_captcha_enable'))) {
                $request->validate([
                    'recaptchaResponse' => 'required',], [
                    'recaptchaResponse.required' => __('Google Captcha is required'),
                ]);
            }
            ...
            if ($user) {
                if ($request->user_type == 1) {
                    ...
                    if ($subscription_details) {
                        $validity_days = (int) $subscription_details?->subscription_type?->validity ?? 0;
                        $expire_date = Carbon::now()->addDays($validity_days);
            ...
            if (Auth::guard('web')->attempt(['username' => $request->username, 'password' => $request->password])) {
                if (Auth::user()->user_type === 1) {
                    return response()->json(['status' => 'client', 'user_id' => Auth::id()]);
                } else {
                    return response()->json(['status' => 'freelancer', 'user_id' => Auth::id()]);
                }
            }
        }
    ...
    public function emailVerify(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if (!is_null($user_details)) {
                ...
                if ($request->ajax() || $request->wantsJson()) {
                    if ($user_details->user_type == 1) {
                        return response()->json(['success' => true, 'redirect' => route('client.profile')]);
                    } else {
                        return response()->json(['success' => true, 'redirect' => route('freelancer.profile')]);
                    }
                }
            ...
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json(['success' => false, 'message' => __('Your verification code is wrong.')], 422);
            }
    ...
    public function resendCode(Request $request)
    {
        ...
        if (!$user_details) {
            ...
            return redirect()->back()->with(['msg' => __('User not found'), 'type' => 'error']);
        }
        ...
        try {
            ...
            $message_body = __('Hello') . ' ' . $user_details->first_name . ' ' . $user_details->last_name . ' <br>' . __('Here is your verification code') . ' <span class="verify-code">' . $verify_token . '</span>';
            ...
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json(['success' => true, 'message' => __('Verification code has been resent to your email')]);
            }

        } catch (\Exception $e) {
            if ($request->ajax() || $request->wantsJson()) {
                return response()->json(['success' => false, 'message' => __('Failed to send email. Please try again.')], 500);
            }
        }
