...
class OrderController extends Controller
{
    ...
    public function user_order_confirm(Request $request)
    {
        ...
        if($project) {
            if ($request->basic_standard_premium_type === $project->basic_title) {
                ...
                $price = ($project->basic_discount_charge !== null && $project->basic_discount_charge > 0)
            ...
            if ($request->basic_standard_premium_type === $project->standard_title) {
                ...
                $price = ($project->standard_discount_charge !== null && $project->standard_discount_charge > 0)
            ...
            if ($request->basic_standard_premium_type === $project->premium_title) {
                ...
                $price = ($project->premium_discount_charge !== null && $project->premium_discount_charge > 0)
        ...
        if (empty($price) || $price == 0) {
            return response()->json(['msg'=> __('Invalid project or job selection')])->setStatusCode('422');
        }
        ...
        {
            // Native Iyzico payment — extract card data from the request
            $cardData = [
    ...
    public function order_rating(Request $request)
    {
        $request->validate([
        ...
        $find_login_user_order = Order::where('id', $request->order_id)
            ->where('user_id', $user_id)
            ->where(function ($q) {
                $q->where('status', 3) // Complete
                    ->orWhere('status', 4); // Cancelled (sometimes rating allowed)
            })->first();

        if (!$find_login_user_order) {
            return response()->json(['msg' => __('Order not found or not eligible for rating.')], 422);
        }

        //check already submit review or not
        $rating = Rating::where('order_id', $find_login_user_order->id)
            ->where('sender_id', $user_id)
            ->where('sender_type', 1) // Client review
            ->first();

        if ($rating) {
            return response()->json(['msg' => __('You have already submitted a review for this order.')], 422);
        }

        $rating = Rating::create([
            'order_id' => $request->order_id,
            'sender_id' => $user_id,
            'sender_type' => 1, // 1 = client review
            'rating' => round($request->rating, 1),
            'review_feedback' => $request->review_feedback,
        ]);

        // Mirroring web multi-criteria rating for compatibility
        $rating_types = ['skill', 'availability', 'communication', 'work-quality', 'deadline', 'co-operation'];
        foreach ($rating_types as $type) {
            \App\Models\RatingDetails::create([
                'rating_id' => $rating->id,
                'type' => $type,
