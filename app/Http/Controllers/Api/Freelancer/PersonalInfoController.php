...
class PersonalInfoController extends Controller
{
    public function personal_info()
    {
        ...
        if ($personal_info) {
            $personal_info->is_pro_tier = is_pro_user($personal_info->id);
            $personal_info->is_premium_tier = is_premium_user($personal_info->id);
        }
    ...
    public function personal_info_update(Request $request)
    {
        $request->validate(
            ...
            \App\Models\UserIntroduction::updateOrCreate(['user_id'=>$user->id],
            [
                'user_id'=>$user->id,
                'title'=>$request->title ?? "",
    ...
    public function profile_image_update(Request $request)
    {
        ...
        if (cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi'])) {
            if ($image = $request->file('image')) {
                $request->validate(
                    ['image'=>'required|mimes:jpg,jpeg,png,gif,svg|max:51200'],
                ...
                // Compress and resize for cloud storage
                $tempPath = tempnam(sys_get_temp_dir(), 'profile');
                Image::make($image)->resize(1000, null, function ($constraint) {
                    $constraint->aspectRatio();
                    $constraint->upsize();
                })->save($tempPath, 80);
                
                $compressedFile = new \Illuminate\Http\File($tempPath);
                add_frontend_cloud_image_if_module_exists($upload_folder, $compressedFile, $imageName, 'public');
                @unlink($tempPath);
            }else{
            ...
            if ($image = $request->file('image')) {
                ...
                // Compress and resize for local storage
                Image::make($image)->resize(1000, null, function ($constraint) {
                    $constraint->aspectRatio();
                    $constraint->upsize();
                })->save('assets/uploads/profile/' . $imageName, 80);
            }else{
                $imageName = $user_image->image;
            }
        }
    ...
    public function profile_details()
    {
        $user = User::with('user_introduction')
            ->select(['id','image','hourly_rate','first_name','last_name','username','country_id','state_id','check_work_availability','load_from', 'phone'])
