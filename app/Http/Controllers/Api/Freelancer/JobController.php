        ...
        if(!empty($request->country) || !empty($request->type) || !empty($request->level) || !empty($request->min_price) || !empty($request->max_price) || !empty($request->duration || !empty($request->category) || !empty($request->subcategory) || !empty($request->string) )){
            ...
            if(!empty($request->string)){
                $jobs->where(function($q) use ($request){
                    $q->where('title','LIKE','%'.$request->string.'%')
                        ->orWhereHas('job_category', function($cq) use ($request){
                            $cq->where('category', 'LIKE', '%'.$request->string.'%');
                        })
                        ->orWhereHas('job_sub_categories', function($sq) use ($request){
                            $sq->where('sub_categories.sub_category', 'LIKE', '%'.$request->string.'%');
                        });
                });
            }
        }
