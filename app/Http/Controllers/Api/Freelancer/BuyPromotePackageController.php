        ...
        if(isset($request->package_id)){
            ...
            if($package_details){
                ...
                elseif($request->selected_payment_gateway == 'iyzipay')
                {
                    ...
                    if($request->type == 'profile' && !empty($find_package_for_profile)){
                        return response()->json(['msg'=> __('Your profile is already in promotion.')])->setStatusCode(422);
                    }
                    if($request->type == 'project' && !empty($find_package_for_project)){
                        return response()->json(['msg'=> __('This project is already in promotion.')])->setStatusCode(422);
                    }
                    ...
                    $callbackUrl = url('/api/v1/freelancer/iyzico/3ds-callback-web?order_id=' . $buy_package->id . '&type=promotion&register_card=' . ($request->register_card ? 1 : 0));
                    
                    $threedsResult = $iyzicoService->initialize3DS($buy_package, $user, $cardData, $callbackUrl);
                    ...
                    if ($threedsResult->getStatus() === 'success') {
                        return response()->json([
                            ...
                            'promotion_details' => $buy_package,
                            'requires_3ds' => true,
                        ]);
                    }

                    $paymentResult = $iyzicoService->processPayment($buy_package, $user, $cardData);
                    if ($paymentResult->getStatus() === 'success') {
                        ...
                        if ($type == 'profile') {
                            User::where('id', $user->id)->update(['is_pro' => 'yes', 'pro_expire_date' => $expire_date]);
                        } else {
                            Project::where('id', $identity)->update(['is_pro' => 'yes', 'pro_expire_date' => $expire_date]);
                        }

                        $this->adminNotification($buy_package->id, $user->id);
                        $this->sendEmail($name, $buy_package->id, $email);
