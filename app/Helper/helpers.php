...
function render_payment_gateway_for_form()
{
    ...
    $all_gateway = ['paypal','manual_payment','mollie','paytm','stripe','razorpay','flutterwave','paystack','marcadopago','instamojo','cashfree','payfast','midtrans','squareup','cinetpay','paytabs','billplz','zitopay','sslcommerce_gateway', 'iyzipay'];
...
function render_payment_gateway_for_form2($except=[])
{
    ...
    $output .= '<input type="hidden" name="selected_payment_gateway" value="' . get_static_option('site_default_payment_gateway') . '">';
    $all_gateway = ['paypal','manual_payment','mollie','paytm','stripe','razorpay','flutterwave','paystack','marcadopago','instamojo','cashfree','payfast','midtrans','squareup','cinetpay','paytabs','billplz','zitopay', 'iyzipay'];
    $output .= '<ul>';
    foreach ($all_gateway as $gateway) {
...
function get_footer_style(){
    // Default to footer 04 if no setting exists
    $fallback = get_static_option('global_footer_variant') ?? '04';
...
function freelancer_notification($identity, $freelancer_id, $type, $msg)
{
    ...
    $notificationBody = [
        ...
        'id' => strval($last_notification->id),
        'identity' => strval($identity),
        'body' => strval($last_notification->message),
        'description' => '',
        'type' => strval($type),
        'sound' => 'default',
        'fcm_device' => ''
        ...
        if($freelancer->firebase_device_token){
            ...
            if (file_exists($credentialsPath)) {
                ...
                $message = CloudMessage::new()
                    ->withNotification(Notification::create($type, $msg))
                ...
                $tokens = is_array($freelancer->firebase_device_token) 
                    ? $freelancer->firebase_device_token 
                    : [$freelancer->firebase_device_token];

                // Use send() per token instead of deprecated sendMulticast()
                foreach ($tokens as $token) {
                    try {
                        $targetMessage = $message->withChangedTarget('token', $token);
                        $result = $messaging->send($targetMessage);
                        Log::info("freelancer_notification: Sent FCM to User: " . $freelancer->id . " | Title: " . $type . " | Msg: " . $msg . " | Token: " . $token . " | Result: " . json_encode($result));
                    } catch (\Exception $sendError) {
                        Log::error("freelancer_notification FCM FAILURE for token " . $token . ": " . $sendError->getMessage());
                    }
                }
            } else {
                Log::error("freelancer_notification: Firebase credentials not found at: " . $credentialsPath);
            }
        }
    }catch (\Exception $e){
        Log::error("freelancer_notification ERROR: " . $e->getMessage());
    }
}
    ...
    try {
        if($client_for_device->firebase_device_token){
            ...
            if (file_exists($credentialsPath)) {
                ...
                $tokens = is_array($client_for_device->firebase_device_token) 
                    ? $client_for_device->firebase_device_token 
                    : [$client_for_device->firebase_device_token];
                ...
                foreach ($tokens as $token) {
                    try {
                        $targetMessage = $message->withChangedTarget('token', $token);
                        $result = $messaging->send($targetMessage);
                        Log::info("client_notification: Sent FCM to User: " . $client_for_device->id . " | Title: " . $type . " | Msg: " . $msg . " | Token: " . $token . " | Result: " . json_encode($result));
                    } catch (\Exception $sendError) {
                        Log::error("client_notification FCM FAILURE for token " . $token . ": " . $sendError->getMessage());
                    }
                }
            } else {
                Log::error("client_notification: Firebase credentials not found at: " . $credentialsPath);
            }
        }
    }catch (\Exception $e){
        Log::error("client_notification ERROR: " . $e->getMessage());
    }
}

function send_voice_call_notification($receiver_id, $data)
{
    $receiver = User::find($receiver_id);
    if (!$receiver) {
        Log::error("send_voice_call_notification: Receiver not found ID: " . $receiver_id);
        return;
    }
    if (!$receiver->firebase_device_token) {
        Log::error("send_voice_call_notification: Missing firebase_device_token for User: " . $receiver->id);
        return;
    }

    $notificationBody = array_merge([
        'click_action' => 'FLUTTER_NOTIFICATION_CLICK',
        'sound' => 'call.mp3', // Future proofing for custom sounds
        'priority' => 'high'
    ], $data);
    ...
    try {
        $credentialsPath = storage_path('app/firebase/firebase_credentials.json');
        if (!file_exists($credentialsPath)) {
            Log::error("send_voice_call_notification: Firebase credentials not found at: " . $credentialsPath);
            return;
        }
        ...
        foreach ($tokens as $token) {
            try {
                ...
                Log::info("send_voice_call_notification: Sent FCM to User: " . $receiver->id . " | Token: " . $token);
            } catch (\Exception $sendError) {
                Log::error("send_voice_call_notification FCM FAILURE for token " . $token . ": " . $sendError->getMessage());
            }
        }
    } catch (\Exception $e) {
...
function project_rating($project_id)
{
    ...
    if ($project_complete_order_count == 0) {
        $project = \App\Models\Project::find($project_id);
        if ($project) {
            $freelancer_rating = \App\Models\Rating::where('sender_type', 1)
                ->whereHas('order', function($q) use ($project) {
                    $q->where('freelancer_id', $project->user_id);
                })
                ->selectRaw('COUNT(id) as rating_count, AVG(rating) as avg_rating')
                ->first();
                
            if ($freelancer_rating && $freelancer_rating->rating_count > 0) {
                $avg_rating = $freelancer_rating->avg_rating;
                $project_complete_order_count = $freelancer_rating->rating_count;
            }
        }
    }
...
{
    // Fetch freelancer rating efficiently
    $result = \App\Models\Rating::where('sender_type', 1)
        ->whereHas('order', function($q) use ($freelancer_id) {
    ...
    $complete_orders_count = \App\Models\Order::where('freelancer_id', $freelancer_id)->where('status', 3)->count();
...
{
    // Fetch levels with proper join and ordering
    $levels = \Modules\FreelancerLevel\Entities\FreelancerLevel::with('level_rule')
        ->join('freelancer_level_rules', 'freelancer_levels.id', '=', 'freelancer_level_rules.freelancer_level_id')
        ->where('freelancer_levels.status', 1)
        ->orderBy('freelancer_level_rules.period', 'desc')
        ->orderBy('freelancer_level_rules.complete_order', 'desc')
        ->orderBy('freelancer_level_rules.earning', 'desc')
        ->select('freelancer_levels.*')
        ->get();
    ...
    // Find the highest level the freelancer qualifies for
    foreach ($levels as $level) {
        $period = $level->level_rule->period ?? 0;
        ...
        // Convert period (months) to days (approximately)
        $required_days = $period * 30;

        // Check if freelancer meets ALL criteria
        if (
            $diff_in_days >= $required_days &&
            ...
            if ($call_from_telent_page == 'talent') {
                return '<div class="level-badge-wrapper">'
                    . render_image_markup_by_attachment_id($level->image)
                    . ' <span class="badge-title">' . $level->level . '</span></div>';
            } else {
                return $level->level;
            }
        }
