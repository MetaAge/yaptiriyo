                                ...
                                children: [
                                  ...
                                  if (gateway != null)
                                    ...Iterable.generate(gateway.field.length)
                                        .map(
                                      (e) {
                                        final isIbanField = gateway.field[e].toLowerCase().contains("iban");
                                        return FieldWithLabel(
                                          label: gateway.field[e],
                                          hintText: isIbanField
                                              ? "TR00-0000-0000-0000-0000-0000-00"
                                              : gateway.field[e].capitalize,
                                          isRequired: true,
                                          controller:
                                              wrm.inputFieldControllers[e],
                                          inputFormatters: isIbanField
                                              ? [
                                          ...
                                          validator: (value) {
                                            ...
                                            if (isIbanField) {
                                              String cleanIban = value
                                                  .replaceAll("-", "")
                                                  .toUpperCase();
                                              if (!RegExp(r'^TR\d{24}$')
                                                  .hasMatch(cleanIban)) {
                                                return "Geçerli bir TR IBAN giriniz (TR + 24 hane)";
                                              }
                                            }
                                            return null;
                                          },
...
class _IbanFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    String text =
        newValue.text.replaceAll(RegExp(r'[^A-Z0-9]'), '').toUpperCase();
    if (text.length > 26) text = text.substring(0, 26);

    StringBuffer buffer = StringBuffer();
    for (int i = 0; i < text.length; i++) {
      buffer.write(text[i]);
      if ((i + 1) % 4 == 0 && (i + 1) != text.length && (i + 1) < 26) {
        buffer.write('-');
      }
    }
