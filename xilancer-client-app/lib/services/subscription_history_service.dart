...
class SubscriptionHistoryService with ChangeNotifier {
  ...
  dynamic get activeSubscriptionId {
    if (_subscriptionHistoryModel == null) return null;
    final now = DateTime.now();
    try {
      final activeSubs = _subscriptionHistoryModel!.allSubscriptions?.history?.firstWhere(
        (element) => 
          element.status.toString() == "1" && 
          (element.expireDate == null || element.expireDate!.isAfter(now)),
      );
      return activeSubs?.subscriptionId;
    } catch (e) {
      return null;
    }
  }
