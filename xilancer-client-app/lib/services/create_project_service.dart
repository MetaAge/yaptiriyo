...
class CreateProjectService with ChangeNotifier {
  tryCreatingProject() async {
    ...
    request.fields.addAll({
      ...
      'video_url': cpm.videoUrlController.text,
      'service_areas': jsonEncode(cpm.selectedServiceAreas.value
          .map((e) => {
                'country_id': e.countryId ?? 15,
                'state_id': e.stateId,
                'city_id': e.cityId,
              })
          .toList()),
    ...
    for (var i = 0; i < cpm.projectImage.value.length; i++) {
      request.files.add(await http.MultipartFile.fromPath(
        'image[]',
        cpm.projectImage.value[i].path,
      ));
    }
  ...
  tryEditingProject() async {
    ...
    if (cpm.removedImages.value.isNotEmpty) {
      request.fields['removed_images'] = jsonEncode(cpm.removedImages.value);
    }
