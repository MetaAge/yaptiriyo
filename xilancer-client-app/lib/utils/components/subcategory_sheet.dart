...
class SubCategorySheet extends StatelessWidget {
  ...
  List<SubCategory> get subCatList {
    try {
      final selectedIds = CreateProjectViewModel
          .instance.selectedSubcategories.value
          .map((e) => e.id.toString())
          .toList();
      final allSubCats = category.value?.subCategories
      ...
      return allSubCats
              .where((element) => (element.subCategory ?? "")
                  .toLowerCase()
                  .contains(subCatString.value.toLowerCase()))
              .toList();
    } catch (e) {
      return [];
    }
