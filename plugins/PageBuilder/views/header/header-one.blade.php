    ...
    <div class="container mx-auto max-w-5xl">
        <div class="flex flex-col items-center text-center space-y-10">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ $find_work_button_link ?? route('projects.all') }}"
            ...
            <div class="flex flex-wrap gap-3 justify-center pt-6">
                @if(isset($skill_tags['tag_name_']) && is_array($skill_tags['tag_name_']) && count($skill_tags['tag_name_']) > 0)
                    @foreach ($skill_tags['tag_name_'] as $key => $tag_name)
                        <a href="{{ $skill_tags['tag_link_'][$key] ?? '#' }}"
