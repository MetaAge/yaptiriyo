...
class WithdrawGatewayController extends Controller
{
    ...
    public function withdraw_request()
    {
        ...
        AdminNotification::where('type','Withdraw')->update(['is_read'=>'read']);
    ...
    public function withdraw_request_update(Request $request)
    {
        ...
        if($request->status == 2) $history_status = 'complete';
        if($request->status == 3) $history_status = 'cancel';

        WalletHistory::where('transaction_id', 'WITHDRAW-' . $withdraw_request->id)->update([
            'payment_status' => $history_status
        ]);
