...
class UserSubscriptionController extends Controller
{
    ...
    public function all_subscription()
    {
        ...
        // For manual assignment modal
        $users = User::select('id', 'first_name', 'last_name', 'username', 'email')->where('user_type', 2)->get();
        $subscriptions = Subscription::with('subscription_type')->where('status', 1)->get();

        return view('subscription::backend.user-subscription.all-subscription',compact(['all_subscriptions','active_subscription','inactive_subscription','manual_subscription','route', 'users', 'subscriptions']));
    }
    ...
    public function assign_subscription(Request $request)
    {
        ...
        $subscription = Subscription::with('subscription_type')->find($request->subscription_id);
        $validity = $subscription->subscription_type->validity ?? 0;
        ...
        UserSubscription::create([
            ...
            'expire_date' => \Carbon\Carbon::now()->addDays($validity),
            'payment_gateway' => 'manual_assignment',
            'payment_status' => 'complete',
            'status' => 1,
            'transaction_id' => 'ADMIN_'.Str::random(10),
        ]);

        return back()->with(toastr_success(__('Subscription assigned successfully')));
    }
    ...
    {
        AdminNotification::where('identity',$id)->where('type','Buy Subscription')->update(['is_read'=>'read']);
    ...
    {
        $subscription_details = UserSubscription::where('id',$request->subscription_id)->first();
        ...
        if($subscription_details){
            $user = User:: select('id','user_type')->where('id',$subscription_details->user->id)->first();
            if($user){
                if($user->user_type == '1'){
                    client_notification($subscription_details->user_id, $user->id,'Subscription','Subscription Payment Confirm');
                    event(new ProjectEvent(__('Subscription Payment Confirm'), $user->id));
                }else{
                    freelancer_notification($subscription_details->user_id, $user->id,'Subscription','Subscription Payment Confirm');
                    event(new ProjectEvent(__('Subscription Payment Confirm'), $user->id));
                }
            }else{
