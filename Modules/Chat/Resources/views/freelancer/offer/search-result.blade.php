...
@else
    @foreach($offers as $offer)
        <div class="myOrder-single bg-white padding-20 radius-10">
            <!-- ADD OFFER TITLE SECTION -->
            <div class="myOrder-single-item">
                <h4 class="myOrder-single-item-title mb-3">
                    @if($offer->title)
                        <span style="color: inherit; font-size: 1.25rem; font-weight: 600;">
                    {{ $offer->title }}
                </span>
                    ...
                    @else
                        <span class="text-danger">{{ __('No Title Available') }}</span>
                    @endif
                </h4>
            </div>
            <!-- END OFFER TITLE SECTION -->
