...
class ProjectController extends Controller
{
    ...
    public function project_details(string $slug) : JsonResponse
    {
        ...
        // Load relationships
        $project->load([
            'project_creator:id,first_name,last_name,username,image,country_id,experience_level,load_from',
            'project_creator' => function($q) {
    ...
    private function applyFilters($query, Request $request): void
    {
        ...
        if ($request->filled('country_id')) {
            $query->where(function($q) use ($request) {
                $q->whereHas('project_creator', function ($q2) use ($request) {
                    $q2->where('country_id', $request->country_id);
                })
                ->orWhereHas('service_areas', function($sq) use($request){
                    $sq->where('country_id', $request->country_id);
                })
