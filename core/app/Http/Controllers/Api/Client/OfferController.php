...
class OfferController extends Controller
{
    ...
    {
        $user_id = auth('sanctum')->user()->id;
        $offer = Offer::where('id',$request->offer_id)->where('client_id',$user_id)->first();
        if($offer){
            $offer->update(['status' => 2]);
            return response()->json(['status' => 1, 'msg' => __('Offer rejected successfully')]);
        }
        return response()->json(['msg'=>__('This offer is not related to you')], 422);
    }
}
