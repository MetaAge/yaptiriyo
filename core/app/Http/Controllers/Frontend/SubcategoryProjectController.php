...
class SubcategoryProjectController extends Controller
{
    ...
    {
        $subcategory = SubCategory::select('id', 'sub_category', 'category_id', 'meta_title', 'meta_description', 'slug')
            ->where('slug', $slug)
            ->first();

        if (!empty($subcategory)) {
            // Get max project price for price range slider
            $maxProjectPrice = $subcategory->projects()
            ...
            $projects = $this->getMixedProjects($request, $subcategory->id);
            ...
            $subcategories = \Modules\Service\Entities\SubCategory::where('category_id', $subcategory->category_id)
                ->where('status', 1)
                ->get();
            $skills = \App\Models\Skill::where('category_id', $subcategory->category_id)
                ->where('status', 1)
                ->get();
            $countries = \Modules\CountryManage\Entities\Country::all_countries();
            $states = \Modules\CountryManage\Entities\State::all_states();

            return view('frontend.pages.subcategory-projects.projects', compact(
                'subcategory', 'projects', 'categories', 'subcategories',
                'skills', 'countries', 'states', 'maxProjectPrice'
            ));
        }
    ...
    public function sub_category_project_filter(Request $request)
    {
        if ($request->ajax()) {
            ...
            $projects = $this->getMixedProjects($request, $subcategoryId);
            $this->trackProProjectImpressions($projects);
            
            if ($projects->total() >= 1) {
                return response()->json([
                    'html' => view('frontend.pages.subcategory-projects.search-subcategory-result', compact('projects'))->render(),
    ...
    {
        $subcategoryId = $request->subcategory_id ?? $request->subcategory;
    ...
    private function getFilteredProjects($request)
    {
        ...
        $query = $subcategory->projects()
            ...
            ->select([
                'projects.id', 'projects.title', 'projects.slug', 'projects.user_id', 'projects.category_id',
                'projects.basic_regular_charge', 'projects.basic_discount_charge',
                'projects.basic_delivery', 'projects.description', 'projects.image', 'projects.load_from',
                'projects.pro_expire_date', 'projects.is_pro', 'projects.created_at'
            ])
            ->withCount(['ratings as ratings_count'])
            ->withAvg('ratings', 'rating')
            ->where('projects.project_on_off', '1')
            ->where('projects.status', '1');

        // Search by text
        if (filled($request->job_search_string)) {
            $query->where('projects.title', 'LIKE', '%' . strip_tags($request->job_search_string) . '%');
        }

        // Country filter
        if (!empty($request->country)) {
            $query->where(function($q) use ($request) {
                $q->where('projects.country_id', $request->country)
        ...
        if (!empty($request->min_price) && !empty($request->max_price)) {
            $query->whereBetween('projects.basic_regular_charge', [$request->min_price, $request->max_price]);
        } elseif (!empty($request->min_price)) {
            $query->where('projects.basic_regular_charge', '>=', $request->min_price);
        } elseif (!empty($request->max_price)) {
            $query->where('projects.basic_regular_charge', '<=', $request->max_price);
        }

        // Delivery day filter
        if (!empty($request->delivery_day)) {
            $query->where('projects.basic_delivery', $request->delivery_day);
        }
        ...
        if (!empty($request->state)) {
            ...
            $query->where(function($q) use ($states) {
                $q->whereIn('projects.state_id', $states)
        ...
        if (!empty($request->city)) {
            ...
            $query->where(function($q) use ($cities) {
                $q->whereIn('projects.city_id', $cities)
        ...
        if (!empty($request->sort_by)) {
            switch ($request->sort_by) {
                ...
                case 'oldest':
                    ...
                    break;
                case 'price_high':
                    $query->orderBy('projects.basic_regular_charge', 'desc');
                    break;
                case 'price_low':
                    $query->orderBy('projects.basic_regular_charge', 'asc');
                    break;
                default:
                    $query->latest();
            }
     ...
     */
    private function getMixedProjects($request, $subcategoryId)
    {
        ...
        if (!$hasPromo) {
            $projects = $this->getProjectsWithoutPromotion($request, $subcategoryId, $perPage);
        ...
        $currentProIds = Project::where('is_pro', 'yes')
            ...
            ->whereHas('project_sub_categories', function($q) use ($subcategoryId) {
     ...
     */
    private function getProjectsWithoutPromotion($request, $subcategoryId, $perPage)
    {
    ...
    private function getProProjectsFirstOptimized($baseQuery, $proIds, $page, $perPage, $request)
    {
        ...
        $totalPro = (clone $baseQuery)->whereIn('projects.id', $proIds)->count();
        $totalNonPro = (clone $baseQuery)->whereNotIn('projects.id', $proIds)->count();
        ...
        if ($offset < $totalPro) {
            ...
            $proQuery = (clone $baseQuery)->whereIn('projects.id', $proIds);
    ...
    private function getMixedProjectsOptimized($baseQuery, $proIds, $page, $perPage, $proCount, $nonProCount, $request)
    {
        ...
        if ($pageData['nonProTake'] > 0) {
            $nonProQuery = (clone $baseQuery)->whereNotIn('projects.id', $proIds);
            ...
            if ($mixed->count() < $perPage) {
                ...
                if ($missingCount > 0) {
                    $extraQuery = (clone $baseQuery)
                        ->whereNotIn('projects.id', $alreadyHaveIds)
                        ->whereNotIn('projects.id', $proIds)
