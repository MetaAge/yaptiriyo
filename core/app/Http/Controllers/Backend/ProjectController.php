...
class ProjectController extends Controller
{
    ...
    public function project_details($id=null)
    {
            ...
            if($project){
                ...
                AdminNotification::where('identity',$id)->where('type','Create Project')->orWhere('type','Edit Project')->update(['is_read'=>'read']);
    ...
    public function change_status($id=null)
    {
        ...
        if($status === 1){
            ...
            Project::where('id',$id)->update(['project_approve_request'=>1]);
                ...
                freelancer_notification($project->id, $project->user_id, 'Activate Project','Your project successfully activate');
                event(new ProjectEvent(__('Your project successfully activate'), $project->user_id));

            try {
                $message = get_static_option('project_approve_email_message') ?? __('Your project successfully activate.');
                $message = str_replace(["@name","@project_title"],[$user->first_name.' '.$user->last_name, $project->title], $message);
            ...
            Project::where('id',$id)->update(['project_approve_request'=>2]);
                freelancer_notification($project->id, $project->user_id, 'Inactivate Project','Your project inactivated. You need to submit again');
                event(new ProjectEvent(__('Your project inactivated. You need to submit again.'),$project->user_id));
            try {
    ...
    public function reject_project(Request $request)
    {
        ...
        event(new ProjectEvent(__('Project rejected by admin'), $project->user_id));
    ...
    // bulk action for project deletion
    public function bulk_action_project(Request $request){
        foreach($request->ids as $project_id){
            ProjectAttribute::where('create_project_id',$project_id)->delete();
            ProjectHistory::where('project_id',$project_id)->delete();
            Bookmark::where('identity',$project_id)->where('is_project_job','project')->delete();
            Project::find($project_id)->delete();
        }

        //security manage
        if(moduleExists('SecurityManage')){
            LogActivity::addToLog('Project bulk delete by admin','Admin');
        }
        return redirect()->back()->with(toastr_success(__('Selected Projects Successfully Deleted with Attributes.')));
    }
}
