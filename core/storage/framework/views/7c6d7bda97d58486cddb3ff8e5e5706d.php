    ...
    <div class="w-full max-w-7xl bg-white rounded-2xl shadow-xl overflow-hidden max-h-[90vh] overflow-y-auto">
        <div class="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div class="relative min-h-[300px] hidden md:block lg:min-h-[700px] bg-cover bg-center overflow-hidden"
                 style="background-image: url('<?php echo e(asset('assets/frontend/new_design/assets/images/login-page/right-side.png')); ?>');">
                <div class="relative z-10 h-full flex flex-col justify-end p-8 text-white">
                    <div class="space-y-4 bg-[#767272]/40 backdrop-blur-xl p-6 rounded-2xl">
                        <h2 class="text-xl lg:text-[1.75rem] font-medium mb-6 leading-[36px]">
                            <?php echo e(get_static_option('login_page_sidebar_title') ?? __('Find the perfect freelance services for your business.')); ?>

                        </h2>
                        <div class="flex items-center gap-3">
                            <i class="fa-regular fa-circle-check text-secondary text-xl"></i>
                            <p class="text-gray-200 text-lg"><?php echo e(__('Discover talented freelancers from around the globe.')); ?></p>
                        </div>
                        <div class="flex items-center gap-3">
                            <i class="fa-regular fa-circle-check text-secondary text-xl"></i>
                            <p class="text-gray-200"><?php echo e(__('Get quality work done quickly and budget.')); ?></p>
                        </div>
                        <div class="flex items-center gap-3">
                            <i class="fa-regular fa-circle-check text-secondary text-xl"></i>
                            <p class="text-gray-200"><?php echo e(__('Manage your projects with our easy-to-use platform.')); ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="p-8 lg:p-12 flex flex-col justify-center relative">
                ...
                <div class="max-w-lg mx-auto w-full">
                    <h1 class="text-xl lg:text-[1.75rem] font-semibold mb-3">
                        <?php echo e(get_static_option('login_page_title') ?? __('Login to Your Account')); ?>
                    ...
                    <form id="modalLoginForm" class="space-y-5">
                        ...
                        <div>
                            <label for="modal_username" class="block text-base font-medium text-base-300 mb-2">
                                <?php echo e(__('Email Or User Name')); ?>

                            </label>
                            <input type="text" id="modal_username" name="username" placeholder="<?php echo e(__('Email Or User Name')); ?>"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="password" id="modal_password" name="password" placeholder="<?php echo e(__('Type password')); ?>"
                        ...
                        <div class="flex items-center justify-between flex-wrap gap-2">
                            <label class="flex items-center gap-2 cursor-pointer">
                                ...
                                <span class="text-sm font-normal text-[#414E62]"><?php echo e(__('Remember Me')); ?></span>
                            </label>
                            <a href="<?php echo e(route('user.forgot.password')); ?>" class="text-sm text-primary font-medium hover:text-teal-700 whitespace-nowrap">
                        ...
                        <button type="submit" id="modalLoginBtn"
                            ...
                            <?php echo e(get_static_option('login_page_button_title') ?? __('Log In')); ?>

                        </button>
                    </form>

                    <!-- Demo Credentials Table -->
                    <?php if(is_test_domain()): ?>
                        <div class="mt-8 border border-gray-200 rounded-lg overflow-hidden">
                            <table class="w-full text-sm">
                                <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-4 py-3 text-left font-semibold text-gray-700"><?php echo e(__('Username')); ?></th>
                                    <th class="px-4 py-3 font-semibold text-gray-700 text-center"><?php echo e(__('Password')); ?></th>
                                    <th class="px-4 py-3 text-end font-semibold text-gray-700"><?php echo e(__('Action')); ?></th>
                                </tr>
                                </thead>
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-end">
                                        <button type="button" class="demo-login-btn bg-primary hover:bg-teal-700 text-white text-xs font-medium px-4 py-2 rounded transition"
                                            ...
                                            <?php echo e(__('Freelancer Login')); ?>
...
<div id="socialRoleModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-[1001] hidden items-center justify-center p-4">
    <div class="relative w-full max-w-5xl z-30 bg-white rounded-3xl overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        <div class="bg-[url('<?php echo e(asset('assets/frontend/new_design/assets/images/login-page/loginpage.png')); ?>')] bg-cover bg-center bg-no-repeat min-h-[400px] flex items-center justify-center p-4 relative">
    ...
    (function($) {
        ...
        $(document).ready(function() {
         ...
         // Open login modal when Sign Up button is clicked (Mobile)
            ...
            $('a[href="<?php echo e(route('user.login')); ?>"]').not('#openLoginModal, #openLoginModalMobile').on('click', function(e) {
                e.preventDefault();
                $('#loginModal').removeClass('hidden').addClass('flex');
                $('body').addClass('overflow-hidden');
            });
            ...
            $('#modalLoginForm').on('submit', function(e) {
                ...
                erContainer.html('');
                btn.text('<?php echo e(__('Please Wait..')); ?>');
                   ...
                   // In success handler:
                    success: function(data) {
                        ...
                        if (data.status == 'client-login' || data.status == 'freelancer-login') {
                            btn.text('<?php echo e(__('Success!')); ?>');
            ...
            $(document).on('click', '.select-social-role', function() {
                ...
                if (selectedUserRole == 2) {
                    $('#confirmSocialRole').text('<?php echo e(__('Join as a Freelancer')); ?>');
                } else {
                    $('#confirmSocialRole').text('<?php echo e(__('Join as a Client')); ?>');
                }
            });
            ...
            $('#confirmSocialRole').on('click', function() {
                if (!selectedUserRole) {
                    alert("<?php echo e(__('Please select account type to continue')); ?>");
                    return;
                }

                let redirectUrl = '';
                if (selectedSocialProvider === 'google') {
                    redirectUrl = '<?php echo e(route('login.google.redirect')); ?>' + '?user_type=' + selectedUserRole;
                } else if (selectedSocialProvider === 'facebook') {
                    redirectUrl = '<?php echo e(route('login.facebook.redirect')); ?>' + '?user_type=' + selectedUserRole;
                }
...
</script><?php /**PATH /Users/ahmetsahin/Desktop/xilancer/core/resources/views/frontend/new_design/layout/partial/login-modal.blade.php ENDPATH**/ ?>