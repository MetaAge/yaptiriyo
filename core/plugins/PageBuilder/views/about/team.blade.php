...
<section class="w-full px-6 py-12 md:py-20 bg-[#F8F9FD]" data-padding-top="{{$padding_top ?? ''}}" data-padding-bottom="{{$padding_bottom ?? ''}}" style="padding-top: {{ $padding_top }}px; padding-bottom: {{ $padding_bottom }}px; background-color:{{$section_bg ?? ''}}">
                        ...
                        <div class="w-full lg:w-1/4 md:w-1/2 flex-shrink-0 px-4">
                            <div class="flex flex-col items-center relative">
                                ...
                                <div class="w-full rounded-xl h-[320px] flex items-start justify-center overflow-hidden mb-4"
                                     style="background-color: {{ $bgColors[$colorIndex % 4] }}">
                                    {!! render_image_markup_by_attachment_id($image, '', 'w-full h-full object-contain mt-5') !!}
                                </div>

                                <!-- Card details -->
                                <div class="bg-base-100 p-6 rounded-lg left-5 right-5 absolute -bottom-10 text-center">
                                    <h3 class="text-[20px] font-medium text-base-300">{{ $repeater_data['name_'][$key] ?? '' }}</h3>
                                    <p class="text-gray-600 text-sm mb-4">{{ $repeater_data['designation_'][$key] ?? '' }}</p>
                                    <div class="flex gap-2 items-center justify-center">
                                        <a href="#"
                                           class="h-5 w-5 flex items-center justify-center transition-colors">
                                            <img class="w-full h-full" src="{{ asset('assets/frontend/new_design/assets/images/about-page/facebook.svg') }}"
                                                 alt="">
                                        </a>
                                        ...
                                        <a href="#"
                                           class="h-5 w-5 flex items-center justify-center transition-colors">
                                            <img class="w-full h-full"
                                                 src="{{ asset('assets/frontend/new_design/assets/images/about-page/instagram.svg') }}" alt="">
                                        </a>
                                        <div class="h-5 border-r border-base-200"></div>
                                        <a href="#"
                                           class="h-5 w-5 flex items-center justify-center transition-colors">
                                            <img class="w-full h-full" src="{{ asset('assets/frontend/new_design/assets/images/about-page/linkedIn.svg') }}"
                                                 alt="">
                                        </a>
                                    </div>
                                </div>
