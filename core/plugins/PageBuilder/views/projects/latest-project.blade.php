@if(get_static_option('project_enable_disable') != 'disable')
    <!-- Trending Services -->
    <section class="relative" style="background-color:{{$section_bg ?? ''}}">
        <div class="container mx-auto max-w-7xl px-4 sm:px-6 py-10 md:py-16 lg:py-28 relative">
            ...
            <h2 class="mb-10 text-4xl text-gray-800 font-medium text-center lg:text-left">
                {{ $title ?? __('Trending Services') }}
            </h2>

            <!-- Carousel Wrapper -->
            <div class="relative px-0">
                <!-- Arrow left -->
                <button title="{{ __('Previous') }}" id="projectPrevBtn"
                ...
                <!-- Arrow right -->
                <button title="{{ __('Next') }}" id="projectNextBtn"
        ...
        {{ get_static_option('promoted_badge_text', 'Sponsored') }}
                                            ...
                                            @if($hasMultipleMedia)
                                                ...
                                                <div class="card-pagination-container">
                                                    @foreach($projectMedia as $dotIndex => $mediaFile)
                                                        <button class="card-pagination-dot w-2 h-2 rounded-full transition-all {{ $dotIndex === 0 ? 'active' : '' }}"
                                                                data-index="{{ $dotIndex }}"
                                ...
                                <div class="p-4 space-y-3">
                                    ...
                                    <div class="flex items-center gap-1 text-yellow-500">
                                        ...
                                        <span class="text-sm font-semibold text-gray-700">{{ $averageRating }}</span>
                                        <span class="text-sm text-gray-500">({{ $ratingCount }} {{ __('Reviews') }})</span>
                                    </div>
                                        ...
                                        <div class="flex items-center gap-3">
                                            <div class="relative w-10 h-10">
                                                @if($project->project_creator?->image)
                                                    @if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                                        <img src="{{ render_frontend_cloud_image_if_module_exists( 'profile/'. $project->project_creator->image, load_from: $project->project_creator->load_from) }}" alt="{{ $project->project_creator->first_name }}" class="w-full h-full rounded-full object-cover border-2 border-white shadow">
                                                    @else
                                                        <img src="{{ asset('assets/uploads/profile/' . $project->project_creator->image) }}" alt="{{ $project->project_creator->first_name }}" class="w-full h-full rounded-full object-cover border-2 border-white shadow">
                                                    @endif
                                                @else
                                                    <img src="{{ asset('assets/static/img/author/author.jpg') }}" alt="{{ __('AuthorImg') }}" class="w-full h-full rounded-full object-cover border-2 border-white shadow">
                                                @endif
                                            ...
                                            <span class="text-sm font-semibold text-gray-700">{{ $project->project_creator?->full_name ?? __('Unknown') }}</span>
                                        </div>
