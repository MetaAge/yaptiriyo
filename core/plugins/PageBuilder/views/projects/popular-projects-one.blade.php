@if(get_static_option('project_enable_disable') != 'disable')
    ...
    <section id="popularServices" class="py-10 md:py-16 lg:py-28 bg-[#F8F9FD]" data-padding-top="{{$padding_top ?? ''}}" data-padding-bottom="{{$padding_bottom ?? ''}}" style="background-color:{{$section_bg ?? ''}}">
        <div class="mx-auto max-w-7xl px-6">
            ...
            <div class="flex flex-col gap-4 lg:flex-row items-center justify-between mb-[40px]">
                ...
                @if(!empty($category_tags) && is_array($category_tags) && count($category_tags) > 0)
                    <div class="flex items-center justify-center flex-wrap gap-2">
                        @foreach($category_tags as $tag)
                            <a href="{{ $tag['tag_url'] ?? '#' }}"
                               ...
                               @if(empty($tag['tag_url'])) onclick="return false;" @endif>
                                {{ $tag['tag_text'] }}
                            </a>
                        @endforeach
                    </div>
                ...
                @foreach($top_projects as $project)
                    ...
                    <div class="card-animate bg-white rounded-[24px] border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all duration-500 w-full overflow-hidden group">
                        <figure class="relative w-full px-3 pt-3">
                            ...
                            <div class="carousel-container relative w-full rounded-lg overflow-hidden" data-carousel-id="carousel-{{ $project->id }}">
                                @if($mediaCount > 0)
                                    ...
                                    @if($hasMultipleMedia)
                                        <button class="arrow-btn left-arrow absolute left-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-2 rounded-full shadow-lg transition-all duration-300 hidden" data-carousel-id="carousel-{{ $project->id }}">
                                        ...
                                        <button class="arrow-btn right-arrow absolute right-2 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white p-2 rounded-full shadow-lg transition-all duration-300" data-carousel-id="carousel-{{ $project->id }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5 text-gray-800">
                                        ...
                                        <div class="pagination-dots-container">
                                            @foreach($projectMedia as $dotIndex => $mediaFile)
                                                <button class="pagination-dot w-2 h-2 rounded-full transition-all {{ $dotIndex === 0 ? 'active' : '' }}"
                                                        data-carousel-id="carousel-{{ $project->id }}"
                                                        data-index="{{ $dotIndex }}"
                            ...
                            @if($isPromotedProject)
                                <span class="bg-[#FA8C00]/90 backdrop-blur-sm rounded-lg text-white px-3 py-1 text-[11px] font-bold uppercase tracking-wider z-10 absolute top-6 left-6 shadow-lg border border-white/20">
                                    {{ get_static_option('promoted_badge_text', __('Öne Çıkan')) }}
                                </span>
                            @endif
                        </figure>
                        ...
                        <div class="px-5 pb-5 pt-2 space-y-3">
                            <hgroup class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <!-- Dynamic Category -->
                                    <p class="text-sm font-medium text-gray-500">{{ $categoryName }}</p>
                                ...
                                <a class="block" href="{{ route('project.details', ['username' => $project->project_creator?->username, 'slug' => $project->slug]) }}">
                             ...
                             <div class="flex items-center gap-1.5">
                                 ...
                                 <span class="text-sm font-bold text-base-300">{{ $formattedAverageRating }}</span>
                             ...
                             <footer class="flex items-center justify-between pt-4 mt-2 border-t border-gray-50">
                                 ...
                                 <div class="flex items-center gap-2.5">
                                     <div class="relative w-9 h-9">
                                         @if(!empty($project->project_creator?->image))
                                             @if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                                 <img src="{{ render_frontend_cloud_image_if_module_exists('profile/'.$project->project_creator?->image, load_from: $project->project_creator?->load_from) }}"
                                         ...
                                         @if($project->project_creator)
                                             ...
                                             @endphp
                                             <div class="absolute -bottom-0.5 -right-0.5 w-3 h-3 {{ $isOnline ? 'bg-green-500' : 'bg-gray-300' }} rounded-full border-2 border-white shadow-sm"></div>
                                         @endif
                                     </div>
                                     <span class="text-[13px] font-medium text-base-300 hover:text-primary transition-colors cursor-pointer">
                                         {{ \Illuminate\Support\Str::limit($project->project_creator?->fullname ?? __('Anonymous'), 15) }}
                                     </span>
                                 </div>
 
                                 <!-- Price -->
                                 <div class="text-right">
                                     <p class="text-[11px] text-gray-400 font-medium uppercase tracking-wider mb-0.5">{{ __('Başlayan fiyatlarla') }}</p>
