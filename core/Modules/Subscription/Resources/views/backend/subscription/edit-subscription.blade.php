                    ...
                    <div class="customMarkup__single__item">
                        ...
                        <div class="customMarkup__single__inner mt-4">
                            ...
                            <form action="{{route('admin.subscription.edit',$subscription_details->id ?? '')}}" method="POST" enctype="multipart/form-data">
                                ...
                                <x-form.text :title="__('Connect')" :type="__('number')" :name="'limit'" :id="'limit'" :divClass="'mb-3'" :value="$subscription_details->limit ?? ''" :placeholder="__('Enter limit')"/>
                                <x-form.text :title="__('Apple Product ID')" :type="__('text')" :name="'apple_product_id'" :id="'apple_product_id'" :value="$subscription_details->apple_product_id ?? ''" :placeholder="__('Enter Apple Product ID')"/>
                                            ...
                                            <div class="attr single-input-feature-attr">
                                                ...
                                                <div class="checkbox-inline">
                                                    <input name="status[{{ $key }}]"
                                                           type="checkbox"
                                                           class="single-input-feature-checkbox check-input"
                                                           value="on"
                                                            {{ $feature->status == 'on' ? 'checked' : '' }}>
                                                    <input type="hidden" name="status_hidden[{{ $key }}]" value="off">
                                                </div>

                                                <button class="btn btn-danger btn-sm remove_row" type="button">
