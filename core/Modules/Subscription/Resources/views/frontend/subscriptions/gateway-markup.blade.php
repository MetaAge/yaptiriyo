            ...
            <div class="modal-content">
                <div class="modal-header">
                    @if(Auth::guard('web')->check())
                        @if(Auth::guard('web')->user()->user_type == 1 && get_static_option('subscription_chat_enable_disable') == 'disable')
                            <h4>{{ __('Buy Subscription') }}</h4>
                        @elseif(Auth::guard('web')->user()->user_type == 1 && Session::get('user_role') != 'freelancer')
                ...
                <div class="modal-footer">
                    ...
                    @if (Auth::guard('web')->check() && Auth::guard('web')->user()->user_type == 1 && Session::get('user_role') == 'freelancer')
                        <button type="submit" class="btn-profile btn-bg-1 buy_subscription" id="confirm_buy_subscription_load_spinner">{{ __('Buy Now') }} <span id="buy_subscription_load_spinner"></span></button>
                    @endif
                    @if (Auth::guard('web')->check() && Auth::guard('web')->user()->user_type == 1 && get_static_option('subscription_chat_enable_disable') == 'disable')
                        <button type="submit" class="btn-profile btn-bg-1 buy_subscription" id="confirm_buy_subscription_load_spinner">{{ __('Buy Now') }} <span id="buy_subscription_load_spinner"></span></button>
                    @endif
                </div>
            </div>
