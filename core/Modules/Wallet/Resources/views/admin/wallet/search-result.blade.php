    ...
    <tbody>
        @foreach ($all_histories as $history)
            <tr>
                <td>{{ $history->id }}</td>
                <td>
                    <p><strong>{{ __('Name:') }}</strong>{{ $history->user?->first_name . ' ' . $history->user?->last_name }}
                    </p>
                ...
                <td>
                    @if ($history->payment_gateway == 'manual_payment' || 'manual_admin')
                    ...
                    @if ($status == 'pending')
                        <div class="btn-group mt-1">
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <x-status.table.status-change :title="__('Complete')" :value="'complete'" :url="route('admin.wallet.history.status', $history->id)"
                                        :class="'dropdown-item swal_status_change_button'" />
                                </li>
                                <li>
                                    <x-status.table.status-change :title="__('Reject')" :value="'reject'" :url="route('admin.wallet.history.status', $history->id)"
                                        :class="'dropdown-item swal_status_change_button'" />
                                </li>
                                <li>
                                    <x-status.table.status-change :title="__('Cancel')" :value="'cancel'"
                                        :url="route('admin.wallet.history.status', $history->id)" :class="'dropdown-item swal_status_change_button'" />
                                </li>
                            </ul>
                        </div>
                    @endif
