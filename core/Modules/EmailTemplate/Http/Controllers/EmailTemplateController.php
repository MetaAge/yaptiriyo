...
class EmailTemplateController extends Controller
{
   ...
   // user info and password update email
    ...
    // admin credit users wallet mail
    public function admin_credit_user_wallet(Request $request)
    {
        if($request->isMethod('post')){
            $request->validate([
                'admin_credit_wallet_subject'=>'required|min:5|max:100',
                'admin_credit_wallet_message'=>'required|min:10|max:100000',
            ]);
            $fields = [
                'admin_credit_wallet_subject',
                'admin_credit_wallet_message',
            ];
            foreach ($fields as $field) {
                update_static_option($field, $request->$field);
            }
        ...
        return view('emailtemplate::wallet.admin-credit-user-wallet');
    }
