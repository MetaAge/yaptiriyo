<table class="DataTable_activation">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                {{ __('Admin Credit User Wallet Email') }}
                <p><small class="text-info">{{__('This email will send to users when admin credit their wallet')}}</small></p>
            </td>
            <td>
                @can('email-template-update')
                <a href="{{ route('admin.user.wallet.credit.email') }}" class="btn-profile btn-bg-1"> {{ __('Edit Template') }}</a>
                @endcan
            </td>
        </tr>
        @endif
