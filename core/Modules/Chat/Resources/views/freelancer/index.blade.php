...
@section('script')
        ...
        $(document).ready(function(){
            ...
            $(document).on('keydown', '#message', function(e) {
                ...
                if (e.keyCode === 13 || e.which === 13) {
                    ...
                    if (e.shiftKey) {
                        return true; // Allow new line
                    } else {
                        // If just Enter (without Shift), send the message
                        e.preventDefault(); // Prevent new line
                        $('#freelancer-send-message-to-client').trigger('click'); // Trigger send button
                        return false;
                    }
                }
            });
    ...
    <script>
        ...
        $(document).on('click','.send_offer_realtime_validation',function(){
            ...
            if(pay_at_once == 'pay-at-once'){
                let offer_description = $('#offer_description').summernote('code');
                let plainText = $('<div>').html(offer_description).text().trim();
                if(offer_revision == '' || offer_deadline == ''){
                    toastr_warning_js("{{ __('Please fill all fields') }}")
                    return false;
                }
                if(!plainText || plainText === ''){
                    toastr_warning_js("{{ __('Please fill description field') }}")
                    return false;
                }
            }
    ...
    <!-- NEW CODE: Handle Enter key to send message -->
    <script>
        $(document).ready(function(){
            // Listen for Enter key press in the message textarea
            $(document).on('keydown', '#message', function(e) {
