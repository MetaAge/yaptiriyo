...
@else
    ...
    @endif
    <a href="{{ route('user.login') }}" class="border border-primary text-base-300 px-4 py-1 rounded-full hover:bg-primary/10 font-medium transition-all duration-300">
        {{ __('Login') }}
    </a>
    <a href="{{ route('user.register') }}" id="openRegisterLink" class="bg-primary text-white text-base-100 px-5 py-1 rounded-full hover:bg-secondary transition-all duration-300 font-medium">
        {{ __('Sign Up') }}
    </a>
@endif