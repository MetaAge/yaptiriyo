        ...
        <form action="{{ route('freelancer.order.submit') }}" method="post" enctype="multipart/form-data" id="submit_order_form">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="orderSubmitModallLabel">{{ __('Complete Service') }}</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <x-notice.general-notice :description="__('Notice: After completing the service, the client will review it and provide final approval. Once approved, the project amount will be added to your account.')" />
                    <div class="error-message"></div>
                    <div class="single-input mt-4">
                        <label class="label-title mb-3">{{ __('Upload Attachment') }} ({{ __('Optional') }})</label>
