    ...
    <div class="popup-fixed profile-popup">
        <div class="popup-contents">
            ...
            <div class="popup-contents-form custom-form profile-border-top">
                ...
                @else
                <div class="single-flex-input">
                    <x-form.state-dropdown :title="__('Select Your Province / State')" :id="'state_id'"/>
                    <x-form.city-dropdown :title="__('Select Your Primary District / City')" :id="'city_id'"/>
                </div>
                <div class="single-input mt-3">
                    <label class="label-title">{{ __('Hizmet Verilen Diğer Bölgeler (İlçeler)') }}</label>
                    <select name="service_areas" id="service_areas" class="form-control service_areas_select2" multiple>
                        @foreach(Auth::guard('web')->user()->service_areas as $area)
                            <option value="{{ $area->city_id }}" selected>{{ optional($area->city)->city }}</option>
                        @endforeach
                    </select>
                </div>
                @endif
