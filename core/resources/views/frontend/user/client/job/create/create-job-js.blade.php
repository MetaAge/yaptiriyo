<script>
    (function($){
        ...
        $(document).ready(function(){
            ...
            function updateCountryPreview() {
                ...
                if (restrictionType === 'none') {
                    previewText =
                        "{{ __('This job is available to freelancers from all countries globally.') }}";
                    ...
                    if (selectedCountries.length > 0) {
                        ...
                        previewText = "{{ __('Only freelancers from') }}: " + countryNames.join(', ') +
                            " {{ __('can see and apply to this job.') }}";
                    } else {
                        previewText = "{{ __('Please select at least one country to allow.') }}";
                    }
                } else if (restrictionType === 'exclude') {
                    ...
                    if (selectedCountries.length > 0) {
                        ...
                        previewText = "{{ __('Freelancers from') }}: " + countryNames.join(', ') +
                            " {{ __('cannot see or apply to this job.') }}";
                    } else {
                        previewText =
                            "{{ __('This job is available to freelancers from all countries.') }}";
                    }
                }
        ...
        $(document).on('input', '#budget', function(){
            ...
            if(value < 0) {
                $(this).val(0);
                toastr_warning_js("{{ __('Budget cannot be negative') }}");
            }
            if(value > 100000) {
                $(this).val(100000);
                toastr_warning_js("{{ __('Budget cannot exceed 100,000') }}");
            }
        });
        ...
        $(document).on('input', '#hourly_rate, #estimated_hours', function(){
            ...
            if(value < 0) {
                $(this).val(1);
                toastr_warning_js("{{ __('Value cannot be negative') }}");
            }
            if(value > 100000) {
                $(this).val(100000);
                toastr_warning_js("{{ __('Value cannot exceed 100,000') }}");
            }
        });
