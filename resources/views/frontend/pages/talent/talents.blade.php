...
@section('content')
    <main>
        <!-- Breadcrumb -->
        <x-breadcrumb.user-profile-breadcrumb-02 :innerTitle="__('All Talents')" />

        <section class="bg-[#F8F9FD]">
            <div class="max-w-7xl mx-auto px-6 py-8 md:py-12 lg:py-16">
                <div class="mb-8">
                    <h1 class="text-3xl font-medium mb-2 text-base-300">{{ __('All Talents') }}</h1>
                    <p class="text-base-400">{{ __('Discover our top rated talents') }}</p>
                </div>
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
                    <div class="flex items-center gap-4">
                        <div id="filter"
                            ...
                            <span class="text-base-400">{{ __('Filter') }}</span>
                        </div>
                        <span class="text-base-400 talent-count-display">{{ __('Showing') }} {{ $talents->count() }} {{ __('out of') }} {{ $talents->total() }} {{ __('results') }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-3 whitespace-nowrap md:self-auto w-full sm:w-auto justify-end">
                        ...
                        <div class="relative min-w-[180px] w-full sm:w-auto">
                            <select title="sort by" id="sort_by"
                                ...
                                <option value="price_high">{{ __('Rate: High to Low') }}</option>
                                <option value="price_low">{{ __('Rate: Low to High') }}</option>
                            </select>
...
@push('page_scripts')
    <script>
        ...
        window.routes.talentsFilter = "{{ route('talents.filter') }}";
        window.routes.talentsPagination = "{{ route('talents.pagination') }}";
        window.routes.talentsFilterReset = "{{ route('talents.filter.reset') }}";
