            ...
            $(document).ready(function () {
                ...
                function filterTalents() {
                    ...
                    $.ajax({
                        ...
                        data: filterData,
                        beforeSend: function() {
                            $('.search_talent_result').html('<div class="col-span-3 text-center py-8"><p class="text-gray-600">{{ __('Loading...') }}</p></div>');
                            $('.talent-count-display').html('{{ __('Loading...') }}');
                        },
                        success: function(res) {
                            ...
                            `);
                                $('.talent-count-display').html('{{ __('Showing 0 results') }}');
                            } else {
                                if (res.html) {
                                    $('.search_talent_result').html(res.html);
                                    $('.talent-count-display').html('{{ __('Showing') }} ' + res.count + ' {{ __('out of') }} ' + res.total + ' {{ __('results') }}');
                                } else {
                ...
                $(document).on('click', '#talent_filter_reset, #clear-all-filters', function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            if (res.status === 'nothing') {
                                $('.search_talent_result').html('<div class="col-span-3 text-center py-8"><h3 class="text-2xl text-red-500">{{ __('Nothing Found') }}</h3></div>');
                                $('.talent-count-display').html('{{ __('Showing 0 results') }}');
                            } else {
                                if (res.html) {
                                    $('.search_talent_result').html(res.html);
                ...
                function updateTalentCountFromDOM() {
                    ...
                    if (talentCards > 0) {
                        $('.talent-count-display').html('{{ __('Showing') }} ' + talentCards + ' {{ __('out of') }} ' + talentCards + ' {{ __('results') }}');
                    }
                }
