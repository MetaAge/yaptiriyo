        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '#category', function() {
                ...
                if (category) {
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            ...
                            if (res.status === 'success') {
                                ...
                                subcategorySelect.empty().append('<option value="">{{ __("Select Subcategory") }}</option>');
                                ...
                                if (res.subcategories && res.subcategories.length > 0) {
                                    ...
                                    $('#subcategory_info').html('<span class="text-red-500 text-sm">{{ __("No subcategories found!") }}</span>');
                                }
                            }
                        },
                        error: function(xhr) {
                    ...
                    $('#subcategory').empty().append('<option value="">{{ __("Select Subcategory") }}</option>');
                    filterJobs();
                }
            });
            ...
            $(document).on('click', '#showMoreBtn', function(e) {
                ...
                if ($extraOptions.hasClass('hidden')) {
                    ...
                    $btn.html('<span>-</span> <span>{{ __("Show Less") }}</span>');
                } else {
                    $extraOptions.addClass('hidden');
                    $btn.html('<span>+</span> <span>{{ __("Show More") }}</span>');
                }
            });
            ...
            function filterJobs() {
                ...
                $.ajax({
                    ...
                    beforeSend: function() {
                        ...
                        $('.search_job_result').html('<div class="col-span-2 text-center py-8"><p class="text-gray-600">{{ __("Loading...") }}</p></div>');
                        $('.job-count-display').html('{{ __("Loading...") }}');
                    },
                        success: function(res) {
                ...
                <section>
                    <div class="flex items-center justify-center min-h-[calc(100vh-171px)] w-full py-10">
                        <div class="max-w-md flex flex-col items-center justify-center">
                            ...
                            <p class="text-base-300 text-2xl">{{ __("Ops! Sorry, no results found.") }}</p>
                        </div>
                    </div>
                </section>
            </div>
        `);
                                $('.job-count-display').html('{{ __("Showing 0 results") }}');
                            } else {
                                console.log('Results found, updating HTML');
                                if (res.html) {
                                    $('.search_job_result').html(res.html);
                                    $('.job-count-display').html('{{ __("Showing") }} ' + res.count + ' {{ __("out of") }} ' + res.total + ' {{ __("results") }}');
                                } else {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        $('.search_job_result').html(`
                            <div class="col-span-2">
                                <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded">
                                    <p class="font-bold">{{ __("Error occurred") }}</p>
                                    <p>{{ __("Please try again or contact support if the issue persists.") }}</p>
                        ...
                        `);
                        $('.job-count-display').html('{{ __("Error loading jobs") }}');
                    }
                });
            }
