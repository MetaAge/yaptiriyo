...
@section('content')
    ...
    <main class="pt-16 md:pt-0"> <!-- Added top padding for mobile -->
        <!-- Main section -->
        <!-- Breadcrumb -->
        <x-breadcrumb.user-profile-breadcrumb-02 :innerTitle="__('Project Details')" />
        <div class="flex flex-col lg:flex-row gap-8 md:gap-12 max-w-7xl mx-auto pt-4 md:pt-10 lg:pt-20 px-4 md:px-6 pb-10 md:pb-20 lg:pb-[120px]">
            ...
            <section class="flex-1 space-y-6 md:space-y-10 min-w-0">
                ...
                <section>
                    ...
                    <!-- Profile Section -->
                    <a href="{{ route('freelancer.profile.details', $user->username) }}" class="block">
                ...
                @if($user->user_verified_status == 1)
                    <i class="fa-solid fa-circle-check text-blue-500 text-xs ml-1" title="{{ __('Verified') }}"></i>
                @endif
                                </span>
                                    @if(moduleExists('FreelancerLevel'))
                                        <span class="border px-3 py-1 rounded-full text-sm">
                  {{ freelancer_level($user->id, 'text') }}
                      </span>
                                    @endif
                                </div>
                                <div class="flex items-center gap-2 mt-1">
                                    ...
                                    <span class="font-medium text-base-300">{{ number_format($ratingValue, 1) }}</span>
                                    <span class="text-base-300">({{ $reviewCount }} reviews)</span>
                                </div>
                            </div>
                        </div>
                    </a>
                                        ...
                                        @forelse($media as $file)
                                            ...
                                            <div class="screen-card">
                                                <div class="w-full h-64 sm:h-80 md:h-96 lg:h-[450px]">
                                                    @if($isImage)
                                                        @if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                                            <img src="{{ render_frontend_cloud_image_if_module_exists('project/'.$file, load_from: $project->load_from) }}"
                                                                 alt="{{ $project->title ?? '' }}"
                                                                 class="w-full h-full object-cover">
                                                        @else
                                                            <img src="{{ asset('assets/uploads/project/'.$file) }}"
                                                                 alt="{{ $project->title ?? '' }}"
                                                                 class="w-full h-full object-cover">
                                                        @endif
                                                    @else
                                                        @if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                                            <video src="{{ render_frontend_cloud_image_if_module_exists('project/'.$file, load_from: $project->load_from) }}"
                                                                   class="w-full h-full object-cover" controls autoplay muted loop playsinline>
                                                                Your browser does not support the video tag.
                                                            </video>
                                                        @else
                                                            <video src="{{ asset('assets/uploads/project/'.$file) }}"
                                                                   class="w-full h-full object-cover" controls autoplay muted loop playsinline>
                                                                Your browser does not support the video tag.
                                                            </video>
                                                        @endif
                            ...
                            <div id="thumbnailContainer" class="flex gap-2 md:gap-4 mb-8 overflow-x-auto pb-2">
                                @foreach($media as $index => $file)
                                    @php
                                        ...
                                        $isImage = in_array(strtolower($ext), ['jpg', 'jpeg', 'png', 'bmp', 'tiff', 'svg', 'webp', 'gif', 'avif']);
                                    @endphp
                                    <div class="thumbnail rounded-lg md:rounded-xl overflow-hidden border-2 border-transparent {{ $index === 0 ? 'active border-primary' : '' }}"
                                         data-index="{{ $index }}"
                ...
                <section class="space-y-[40px]">
                    ...
                    @if($project->project_attributes->count() > 0)
                        <div>
                            ...
                            <ul class="space-y-3">
                                @foreach($project->project_attributes as $attr)
                                    @if($attr->basic_check_numeric == 'on' || $attr->standard_check_numeric == 'on' || $attr->premium_check_numeric == 'on')
                                        <li class="flex items-center justify-start">
                                            <span class="text-[#3B4759] mr-2">●</span>
                                            <span class="text-base-400">{{ $attr->check_numeric_title }}</span>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
                    ...
                    <div class="bg-white rounded-2xl border border-[#C4C8CE] p-6 mb-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center justify-between">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex flex-col gap-1 md:block">
                                            ...
                                            <div class="flex items-center gap-1 md:gap-2">
                                                ...
                                                <span class="font-medium text-base-300 text-sm md:text-base">{{ number_format($ratingValue, 1) }}</span>
                                                <span class="text-base-300 text-sm md:text-base">({{ $reviewCount }} {{ __('reviews') }})</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            @if (Auth::guard('web')->check() && Auth::guard('web')->user()->user_type == 1 && Auth::guard('web')->user()->id != $project->user_id && Session::get('user_role') != 'freelancer')
                                <a href="{{ route('client.live.chat') }}?freelancer_id={{ $project->user_id }}&project_id={{ $project->id }}"
                        ...
                        <div class="grid grid-cols-2 gap-6 mb-4 mt-6">
                            <div>
                                <p class="text-sm text-gray-600 mb-1">{{ __('From') }}</p>
                            ...
                            <div class="justify-self-center">
                                <p class="text-sm text-gray-600 mb-1">{{ __('Member Since') }}</p>
                                <p class="font-medium text-base-300">
                            ...
                            @if($user->completed_orders_count > 0)
                                <div>
                                    <p class="text-sm text-gray-600 mb-1">{{ __('Jobs Completed') }}</p>
                                    <p class="font-medium text-base-300">
                        ...
                        @if($user->freelancer_skill->count() > 0)
                            @php
                        ...
                        <div class="bg-white rounded-2xl border border-[#C4C8CE] overflow-auto">
                            <table class="w-full">
                                ...
                                <tr class="text-base-300 bg-primary/5">
                                    ...
                                    <td class="text-center p-4 border-r">{{ $project->basic_revision }}</td>
                                    @if(!empty($project->standard_title))
                                        <td class="text-center p-4 border-r">{{ $project->standard_revision }}</td>
                                    @endif
                                    @if(!empty($project->premium_title))
                                        <td class="text-center p-4">{{ $project->premium_revision }}</td>
                                    @endif
                                </tr>
                                ...
                                <tr class="">
                                    ...
                                    <td class="text-center p-4 text-sm text-base-300 border-r">{{ $project->basic_delivery }}</td>
                                    @if(!empty($project->standard_title))
                                        <td class="text-center p-4 text-sm text-base-300 border-r">{{ $project->standard_delivery }}</td>
                                    @endif
                                    @if(!empty($project->premium_title))
                                        <td class="text-center p-4 text-sm text-base-300">{{ $project->premium_delivery }}</td>
                                    @endif
                                </tr>

                                <!-- Attributes Rows -->
                                @foreach($project->project_attributes as $attr)
                                    <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-primary/5' : '' }}">
                                        <td class="border-r p-4 text-sm text-base-300">{{ $attr->check_numeric_title }}</td>
                ...
                <!-- Review Section -->
                @if($project_complete_orders->count() > 0)
                    <section>
                        <h2 class="text-xl lg:text-2xl text-base-300 font-medium mb-4">
                            Reviews ({{ $project_complete_orders->total() }})
                        </h2>
            ...
            <section class="lg:w-96 xl:w-[440px] flex-shrink-0">
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden sticky top-[100px] border">
                    ...
                    <div class="flex border-b">
                        <div class="package-tab active flex-1 text-center py-3" data-package="basic">{{ $project->basic_title }}</div>
                        @if(!empty($project->standard_title))
                            <div class="package-tab flex-1 text-center py-3" data-package="standard">{{ $project->standard_title }}</div>
                        @endif
                        @if(!empty($project->premium_title))
                            <div class="package-tab flex-1 text-center py-3" data-package="premium">{{ $project->premium_title }}</div>
                        @endif
                    </div>
                            ...
                            <div class="mb-6">
                                ...
                                <div class="flex items-center gap-4 md:gap-10 mb-6">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-base-300 font-medium">{{ $project->basic_delivery }} delivery</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <i class="icon-base ti tabler-refresh-dot icon-20px text-base-300 font-medium"></i>
                                        <span class="text-base-300 font-medium">{{ $project->basic_revision }} Revision</span>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-3 text-sm">
                                    @foreach($project->project_attributes as $attr)
                                        @if($attr->basic_check_numeric == 'on' && empty($attr->basic_extra_price))
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex items-center justify-between mb-2">
                                                    <div class="flex items-center gap-2">
                                                        <input type="checkbox" class="basic-extra-checkbox rounded text-primary"
                                                               ...
                                                               name="extras[{{ $attr->id }}]">
                                                        <span class="text-base-400 text-sm">{{ $attr->check_numeric_title }}</span>
                                                    </div>
                                                    <span class="text-sm text-base-300">+{{ float_amount_with_currency_symbol($attr->basic_extra_price) }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    @endif
                            ...
                            <div id="standardPackage" class="package-content hidden" data-base-price="{{ $standardPrice }}">
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="flex items-center gap-4 md:gap-10 mb-6">
                                        <div class="flex items-center gap-2">
                                            <i class="icon-base ti tabler-clock-share icon-20px text-base-300 font-medium"></i>
                                            <span class="text-base-300 font-medium">{{ $project->standard_delivery }} delivery</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <i class="icon-base ti tabler-refresh-dot icon-20px text-base-300 font-medium"></i>
                                            <span class="text-base-300 font-medium">{{ $project->standard_revision }} Revision</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-3 text-sm">
                                        @foreach($project->project_attributes as $attr)
                                            @if($attr->standard_check_numeric == 'on' && empty($attr->standard_extra_price))
                                                <div class="flex items-center gap-2">
                                                    <i class="icon-base ti tabler-check icon-18px text-base-300 font-medium"></i>
                                                    <span class="text-base-400">{{ $attr->check_numeric_title }}</span>
                                                </div>
                                            ...
                                            <div class="mt-4 pt-4 border-t border-gray-200">
                                                ...
                                                @foreach($standardExtras as $attr)
                                                    <div class="flex items-center justify-between mb-2">
                                                        ...
                                                        <span class="text-sm text-base-300">+{{ float_amount_with_currency_symbol($attr->standard_extra_price) }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endif
                            ...
                            <div id="premiumPackage" class="package-content hidden" data-base-price="{{ $premiumPrice }}">
                                ...
                                <div class="mb-6">
                                    ...
                                    <div class="flex items-center gap-4 md:gap-10 mb-6">
                                        <div class="flex items-center gap-2">
                                            <i class="icon-base ti tabler-clock-share icon-20px text-base-300 font-medium"></i>
                                            <span class="text-base-300 font-medium">{{ $project->premium_delivery }} delivery</span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <i class="icon-base ti tabler-refresh-dot icon-20px text-base-300 font-medium"></i>
                                            <span class="text-base-300 font-medium">{{ $project->premium_revision }} Revision</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-3 text-sm">
                                        @foreach($project->project_attributes as $attr)
                                            @if($attr->premium_check_numeric == 'on' && empty($attr->premium_extra_price))
                                                <div class="flex items-center gap-2">
                                                    <i class="icon-base ti tabler-check icon-18px text-base-300 font-medium"></i>
                                                    <span class="text-base-400">{{ $attr->check_numeric_title }}</span>
                                                </div>
                                            ...
                                            <div class="mt-4 pt-4 border-t border-gray-200">
                                                ...
                                                @foreach($premiumExtras as $attr)
                                                    <div class="flex items-center justify-between mb-2">
                                                        ...
                                                        <span class="text-sm text-base-300">+{{ float_amount_with_currency_symbol($attr->premium_extra_price) }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endif
        ...
        @include('frontend.pages.project-details.related-service')
    </main>
@endsection
...
@section('script')
    ...
    <script>
        // Global variables
        window.siteCurrency = '{{ get_static_option("site_global_currency") ?? "$" }}';
        window.reviewLoadMoreUrl = "{{ route('project.review.load.more') }}";
...
// Initialize on page load and bind events
            ...
            $('#continueOrderBtn').on('click', function(e) {
                ...
                const url = new URL('{{ route("order.checkout.page") }}', window.location.origin);
