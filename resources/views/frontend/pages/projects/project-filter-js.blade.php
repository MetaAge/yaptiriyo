            ...
            $(document).ready(function () {
                ...
                $(document).on('change', '#sidebar #category, #category', function() {
                    ...
                    if (category) {
                        ...
                        $('#sidebar #subcategory').empty().append('<option value="">{{ __('Select Subcategory') }}</option>');
                    }
                });
                ...
                $(document).on('change', '#sidebar #state, #state', function() {
                    ...
                    if (state) {
                        ...
                        $('#sidebar #city').empty().append('<option value="">{{ __('Select City') }}</option>');
                    }
                });
...
// Keep Enter key functionality for accessibility
                ...
                function filterProjects() {
                    ...
                    $.ajax({
                        ...
                        data: filterData,
                        beforeSend: function() {
                            $('.search_project_result').html('<div class="col-span-2 text-center py-8"><p class="text-gray-600">{{ __('Loading...') }}</p></div>');
                ...
                $(document).on('click', '#project_filter_reset, #clear-all-filters', function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(res) {
                            if (res.status === 'nothing') {
                                $('.search_project_result').html('<div class="col-span-2 text-center py-8"><h3 class="text-2xl text-red-500">{{ __('Nothing Found') }}</h3></div>');
                                $('.project-count-display').html('{{ __('Showing 0 results') }}');
                            } else {
                                if (res.html) {
                                    $('.search_project_result').html(res.html);
            ...
            // ALSO INITIALIZE ON PAGE LOAD
                ...
                function updateProjectCountFromDOM() {
                    ...
                    if (projectCards > 0) {
                        $('.project-count-display').html('{{ __('Showing all') }} ' + projectCards + ' {{ __('results') }}');
                    }
                }
