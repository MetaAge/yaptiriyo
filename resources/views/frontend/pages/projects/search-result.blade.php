        ...
        <div class="card-animate bg-white rounded-2xl shadow-lg w-full overflow-hidden">
            <figure class="relative w-full px-4 pt-4">
                ...
                <x-frontend.bookmark2
                        :identity="$project->id"
                        :type="'project'"
                        :btnClass="'absolute top-6 right-6 p-2 bg-white rounded-full hover:text-red-500 transition-colors z-20'" />
            </figure>
            ...
            <div class="p-4 space-y-3">
                <hgroup class="space-y-3">
                    <div class="flex items-center justify-between">
                        ...
                        @if(!empty($freelancerLevel))
                            {{-- Display freelancer level badge only if FreelancerLevel module exists --}}
                            <p class="bg-secondary/20 px-2 py-1 rounded-lg text-secondary text-xs font-medium">
                                {{ $freelancerLevel }}
                            </p>
                        @elseif($isPromotedProject)
                            {{-- Display promoted badge if project is promoted --}}
                            <p class="bg-secondary/20 px-2 py-1 rounded-lg text-secondary text-xs font-medium">
                                {{ get_static_option('promoted_badge_text', 'Sponsored') }}
                            </p>
                        @elseif($project->created_at && $project->created_at->diffInHours(now()) < 24)
                            {{-- Display "New" badge for recently created projects --}}
                ...
                <div class="flex items-center justify-between text-yellow-500">
                    <div class="flex items-center gap-1">
                        @if($project->ratings_count > 0)
                            ...
                            <span class="text-sm font-semibold text-gray-700">{{ number_format($project->average_rating ?? ($project->ratings_avg_rating ?? 0), 1) }}</span>
                            <span class="text-sm text-gray-500">({{ $project->ratings_count }})</span>
                        @else
                            <span class="text-sm text-gray-500">{{ __('No reviews yet') }}</span>
                        @endif
                    </div>
                    @if($project->project_creator?->completed_orders_count > 0)
                        <div class="flex items-center gap-1 text-gray-500">
                            <i class="fa-solid fa-briefcase text-xs"></i>
                            <span class="text-xs font-medium">{{ $project->project_creator->completed_orders_count }} {{ __('Jobs Done') }}</span>
                        </div>
                    @endif
                </div>

                @if($project->state || $project->city || $project->project_creator?->state)
                    <div class="flex items-center gap-1 text-gray-500 text-xs">
                        <i class="fa-solid fa-location-dot"></i>
                        <span>
                            {{ $project->city?->city ?? $project->state?->state ?? $project->project_creator?->state?->state ?? __('Turkey') }}
                        </span>
                    </div>
                @endif
                ...
                <footer class="flex items-center justify-between pt-3 border-t border-gray-200">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-center gap-1">
                            <span class="text-sm font-semibold text-gray-700">{{ $project->project_creator?->fullname ?? __('User') }}</span>
                            @if($project->project_creator?->user_verified_status == 1)
                    ...
                    <div class="text-right flex items-center gap-1">
                        ...
                        <p class="text-lg font-semibold text-gray-800">
                            {{ yaptiriyo_price_label($project->basic_discount_charge > 0 ? $project->basic_discount_charge : $project->basic_regular_charge, $project->pricing_type, true) }}
                        </p>
                    </div>
                </footer>
            </div>
                ...
                <div class="flex items-center justify-center h-full w-full py-10">
                    <div class="max-w-lg flex flex-col items-center justify-center text-center">
                        ...
                        <button
                            ...
                            <a href="{{ route('freelancer.project.create') }}">Create a job</a>
