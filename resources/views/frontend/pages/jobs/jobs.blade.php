...
@section('content')
    ...
    <main>
        ...
        <!-- Breadcrumb -->
        <x-breadcrumb.user-profile-breadcrumb-02 :innerTitle="__('All Jobs')" />

        <section class="bg-[#F8F9FD]">
            <div class="max-w-7xl mx-auto px-6 py-8 md:py-12 lg:py-16">
                <div class="mb-8">
                    <h1 class="text-3xl font-medium mb-2">{{ __('All Jobs') }}</h1>
                    <p class="text-gray-600">{{ __('Browse all available freelance projects') }}</p>
                </div>
...
@push('page_scripts')
    <script>
        window.routes = {
            ...
            jobsFilter: "{{ route('jobs.filter') }}",
            jobsPagination: "{{ route('jobs.pagination') }}",
            jobsFilterReset: "{{ route('jobs.filter.reset') }}"
        };
