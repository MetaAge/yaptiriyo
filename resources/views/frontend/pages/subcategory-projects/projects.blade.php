...
@section('content')
    <main>
        <!-- Breadcrumb -->
        <x-breadcrumb.user-profile-breadcrumb-02 :title="$subcategory->sub_category ?? __('Project Category')" :innerTitle="$subcategory->sub_category ?? '' " />

        <section class="bg-[#F8F9FD]">
            <div class="max-w-7xl mx-auto px-6 py-8 md:py-12 lg:py-16">
                <div class="mb-8">
                    <h1 class="text-3xl font-medium mb-2 text-base-300">{{ $subcategory->sub_category ?? __('Sub Category Projects') }}</h1>
                    <p class="text-base-400">{{ __('Browse all services in this sub category') }}</p>
                </div>
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 overflow-hidden">
                    <div class="flex items-center gap-4">
                        ...
                        <span class="text-base-400 project-count-display">{{ __('Showing') }} {{ $projects->count() }} {{ __('out of') }} {{ $projects->total() }} {{ __('results') }}</span>
                    </div>
