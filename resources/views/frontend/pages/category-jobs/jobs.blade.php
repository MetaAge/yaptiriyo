...
@section('site_title', $category->category . ' ' . __('Jobs'))
@section('meta_title'){{ $category->category . ' ' . __('Jobs') }}@endsection

@section('content')
    <main>
        <!-- Breadcrumb -->
        <x-breadcrumb.user-profile-breadcrumb-02
                :innerTitle="$category->category . ' ' . __('Jobs')"
                :breadcrumbTitle="$category->category"
        />

        <section class="bg-[#F8F9FD]">
            <div class="max-w-7xl mx-auto px-6 py-8 md:py-12 lg:py-16">
                <div class="mb-8">
                    <h1 class="text-3xl font-medium mb-2">{{ $category->category }} {{ __('Jobs') }}</h1>
                    <p class="text-gray-600">{{ __('Browse all available') }} {{ strtolower($category->category) }} {{ __('projects') }}</p>
                </div>
...
@push('page_scripts')
    <script>
        window.routes = {
            ...
            jobsFilter: "{{ route('category.jobs.filter') }}",
            jobsPagination: "{{ route('category.jobs.pagination') }}",
            jobsFilterReset: "{{ route('category.jobs.filter.reset') }}"
        };
        window.csrfToken = "{{ csrf_token() }}";
        window.maxJobPrice = {{ $maxJobPrice ?? 1000 }};
    </script>
