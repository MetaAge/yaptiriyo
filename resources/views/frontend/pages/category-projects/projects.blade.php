...
@section('site_title', $category->meta_title ?? $category->category ?? 'Kategori Hizmetleri')
@section('meta_title'){{ $category->meta_title ?? $category->category ?? 'Kategori Hizmetleri' }}@endsection
@section('meta_description'){{ $category->meta_description ?? '' }}@endsection

@section('content')
    <main>
        <!-- Breadcrumb -->
        <x-breadcrumb.user-profile-breadcrumb-02 :innerTitle="$category->category ?? 'Kategori Hizmetleri'" />

        <section class="bg-[#F8F9FD]">
            <div class="max-w-7xl mx-auto px-6 py-8 md:py-12 lg:py-16">
                <div class="mb-8">
                    <h1 class="text-3xl font-medium mb-2 text-base-300">{{ $category->category ?? 'Kategori Hizmetleri' }}</h1>
...
@push('page_scripts')
    <script>
        ...
        window.routes.categoryProjectsFilter = "{{ route('category.projects.filter') }}";
        window.routes.categoryProjectsPagination = "{{ route('category.project.pagination') }}";
        window.routes.categoryProjectsReset = "{{ route('category.project.filter.reset') }}";
