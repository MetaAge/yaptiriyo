...
<head>
    ...
    <title>{{ $reel->description ?? 'Reel Video' }} | Yaptiriyo</title>
    
    <!-- Meta Tags for Social Sharing -->
    <meta property="og:title" content="{{ $reel->description ?? 'Reel Video' }}">
    <meta property="og:description" content="Watch this amazing reel on Yaptiriyo!">
    <meta property="og:image" content="{{ asset('assets/uploads/reels/thumbnails/'.$reel->thumbnail) }}">
    <meta property="og:video" content="{{ asset('assets/uploads/reels/'.$reel->video) }}">
...
<body>
    <div class="reel-container">
        ...
        <video id="reelVideo" loop playsinline autoplay muted>
            <source src="{{ asset('assets/uploads/reels/'.$reel->video) }}" type="video/mp4">
        </video>
        
        <div class="overlay">
            <div class="overlay-content">
                <div class="user-info">
                    <img src="{{ $reel->user?->image ? asset('assets/uploads/profile/'.$reel->user->image) : 'https://www.gravatar.com/avatar/' }}" class="user-avatar" alt="">
                    <span class="username">@ {{ $reel->user?->username ?? 'freelancer' }}</span>
                </div>
