                                                ...
                                                <div class="row g-0">
                                                    <div class="col-md-4 {{ ($project->type ?? '') == 'job'?'d-none' : '' }}">
                                                        ...
                                                        @else
                                                            ...
                                                            @if($firstImage)
                                                                @if(cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3', 'cloudFlareR2', 'wasabi']))
                                                                    <img class="img-fluid rounded-start" src="{{ render_frontend_cloud_image_if_module_exists( 'project/'. $firstImage, load_from: $project->load_from ?? '') }}" alt="{{ $project->title ?? '' }}">
                                                                @else
                                                                    <img src="{{ asset('assets/uploads/project/'.$firstImage) }}" class="img-fluid rounded-start" alt="{{ $project->title ?? '' }}">
                                                                @endif
                                                            @endif
                                                        @endif
                                                    </div>
