  ...
  fetchProjectDetails(id) async {
    ...
    if (responseData != null) {
      ...
      tempProject?.projectAttributes?.forEach((element) {
        basicPackage.extraFields.add(
          ExtraFieldForView(
            id: element.id,
            name: element.checkNumericTitle ?? "",
            type:
                element.type.toString() == "checkbox"
                    ? FieldType0.CHECK
                    : FieldType0.QUANTITY,
      ...
      if ((tempProject?.standardRegularCharge ?? 0) > 0) {
        ...
        tempProject?.projectAttributes?.forEach((element) {
          standardPackage.extraFields.add(
            ExtraFieldForView(
              id: element.id,
              name: element.checkNumericTitle ?? "",
              type:
      ...
      if ((tempProject?.premiumRegularCharge ?? 0) > 0) {
        ...
        tempProject?.projectAttributes?.forEach((element) {
          premiumPackage.extraFields.add(
            ExtraFieldForView(
              id: element.id,
              name: element.checkNumericTitle ?? "",
              type:
