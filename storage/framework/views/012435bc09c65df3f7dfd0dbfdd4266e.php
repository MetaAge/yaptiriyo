<script>
    function loadSkillsByCategory(categoryId) {
        $.ajax({
            method: 'post',
            url: "<?php echo e(route('au.skills.by.category')); ?>",
            data: {
                category: categoryId,
            ...
            success: function(res) {
                if (res.status == 'success') {
                    ...
                    if (res.skills.length <= 0) {
                        $('.setup-wrapper-work-list').html('<li class="text-danger"><?php echo e(__("No skills found for selected category!")); ?></li>');
                    }
                } else {
                    $('.setup-wrapper-work-list').html('<li class="text-danger"><?php echo e(__("Failed to load skills")); ?></li>');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error loading skills:', error, xhr.responseText);
                $('.setup-wrapper-work-list').html('<li class="text-danger"><?php echo e(__("Error loading skills")); ?></li>');
            }
        });
    }
     function loadSubcategoriesByCategory(categoryId) {
        $.ajax({
            ...
            success: function(res) {
                if (res.status == 'success') {
                    ...
                    if(all_subcategories.length <= 0){
                        $(".get_subcategory").html('<span class="text-danger"> <?php echo e(__('No subcategory found for selected category!')); ?> <span>');
                    }else{
            ...
            error: function(xhr, status, error) {
                ...
                $('.get_subcategory').html('<li class="text-danger"><?php echo e(__("Error loading subcategories")); ?></li>');
            }
        });
    }
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            $(document).on('click','.add_experience',function(){
                ...
                if(short_description.length > 255){
                    toastr_warning_js("<?php echo e(__('Short description must not greater than 255 charecter !')); ?>");
                    return false;
                }
            ...
            $(document).on('click','.add_education',function(){
                ...
                if(startDate > today){
                    toastr_warning_js("<?php echo e(__('Start date cannot be in the future!')); ?>");
                    return false;
                }
                ...
                if(end_date != ''){
                    ...
                    if(endDate < startDate){
                        toastr_warning_js("<?php echo e(__('End date cannot be before start date!')); ?>");
                        return false;
                    }
                }
            ...
            $(document).on('keyup','#category_search_string', function (){
                ...
                $.ajax({
                    url:"<?php echo e(route('freelancer.account.category.search')); ?>",
                    method:'GET',
            ...
            $(document).on('submit','#profilePhotoUploadForm', function(e) {
                ...
                $.ajax({
                    url:"<?php echo e(route('freelancer.account.profile.photo.upload')); ?>",
                    method:'post',
                ...
                if (current <= Listings.length - 1) {
                    ...
                    if(current == 1){
                        ...
                        if(title == '' || description == ''){
                            current = 0;
                            toastr_warning_js("<?php echo e(__('Please fill title and description !')); ?>");
                            return false;
                        }else if(description.length <50){
                            current = 0;
                            toastr_warning_js("<?php echo e(__('Description must be at least 50 character !')); ?>");
                            return false;
                        }else if(description.length >1500){
                            current = 0;
                            toastr_warning_js("<?php echo e(__('Description must not greater than 1500 character !')); ?>");
                            return false;
                        }else{
                            ...
                            $.ajax({
                                url: "<?php echo e(route('freelancer.account.introduction.add')); ?>",
                                type: 'post',
                                data: {title: title, description: description},
                                success: function(res){
                                    if(res.status == 'ok'){
                                        toastr_success_js("<?php echo e(__('Introduction Successfully Updated')); ?>");
                                ...
                                error: function(xhr, status, error) {
                                    current = 0;
                                    toastr_warning_js("<?php echo e(__('An error occurred. Please try again.')); ?>");
                                    return false;
                                }
                            });
                        }
                    ...
                    // add work
                        ...
                        if(category == ''){
                            current = 3;
                            toastr_warning_js("<?php echo e(__('Please choose a category !')); ?>");
                            return false;
                        }else{
                            $.ajax({
                                url: "<?php echo e(route('freelancer.account.work.add')); ?>",
                                type: 'post',
                                data: {category: category,subcategory:subcategory},
                                success: function(res){
                                    if(res.status == 'ok'){
                                        toastr_success_js("<?php echo e(__('Work Successfully Updated')); ?>");
                                    }
                                }
                            });
                        }
                    ...
                    // add skill
                        ...
                        if(skill == ''){
                            current = 4;
                            toastr_warning_js("<?php echo e(__('You must add one or more skills !')); ?>");
                            return false;
                        }else{
                            $.ajax({
                    ...
                    // add hourly rate
                        ...
                        if(hourly_rate == ''){
                            current = 5;
                            toastr_warning_js("<?php echo e(__('You must add hourly rate!')); ?>");
                            return false;
                        }
                        ...
                        if(hourly_rate < 0){
                            current = 5;
                            toastr_warning_js("<?php echo e(__('Hourly rate cannot be negative!')); ?>");
                            return false;
                        }
                       ...
                       let conversion_rate = 0;
                           conversion_rate = <?php echo e(10000000 * $get_user_currency->conversion_rate ?? 1); ?>;
                        if(hourly_rate > conversion_rate){
                            current = 5;
                            toastr_warning_js("<?php echo e(__('Amount must not greater then')); ?>" + " " + conversion_rate);
                            return false;
                        }else{
                            $.ajax({
                                url: "<?php echo e(route('freelancer.account.hourly.rate.add')); ?>",
                                type: 'post',
                                data: {hourly_rate: hourly_rate},
                                success: function(res){
                                    if(res.status == 'ok'){
                                        toastr_success_js("<?php echo e(__('Profile Photo Successfully Updated')); ?>");
                                        let redirectPath = "<?php echo e(route('freelancer.account.congrats')); ?>";
                        ...
                        if(hourly_rate > 10000000){
                            current = 5;
                            toastr_warning_js("<?php echo e(__('Amount must not greater then 10000000')); ?>");
                            return false;
                        }else{
                            $.ajax({
                                url: "<?php echo e(route('freelancer.account.hourly.rate.add')); ?>",
...
<?php /**PATH /Users/ahmetsahin/Desktop/xilancer/core/resources/views/frontend/user/freelancer/account/account-setup-js.blade.php ENDPATH**/ ?>