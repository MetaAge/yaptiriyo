<script>
    (function($){
        ...
        $(document).ready(function(){
            ...
            // Initialize Select2 for country restriction dropdowns
            $('.allowed_countries_select2').select2();

            $('.excluded_countries_select2').select2();

            // Handle country restriction type change
            $('#country_restriction_type').on('change', function() {
                const restrictionType = $(this).val();

                // Hide all country selection wrappers
                $('.country-selection-wrapper').hide();
                $('#country_preview').hide();

                // Clear selections
                $('#allowed_countries').val(null).trigger('change');
                $('#excluded_countries').val(null).trigger('change');

                // Show appropriate wrapper based on selection
                if (restrictionType === 'include') {
                    $('#include_countries_wrapper').show();
                } else if (restrictionType === 'exclude') {
                    $('#exclude_countries_wrapper').show();
                }
            ...
            $('#allowed_countries, #excluded_countries').on('change', function() {
                updateCountryPreview();
            });

            // Function to update country restriction preview
            function updateCountryPreview() {
                const restrictionType = $('#country_restriction_type').val();
                let previewText = '';

                if (restrictionType === 'none') {
                    previewText =
                        "<?php echo e(__('This job is available to freelancers from all countries globally.')); ?>";
                } else if (restrictionType === 'include') {
                    const selectedCountries = $('#allowed_countries option:selected');
                    if (selectedCountries.length > 0) {
                        const countryNames = selectedCountries.map(function() {
                            return $(this).text();
                        }).get();
                        previewText = "<?php echo e(__('Only freelancers from')); ?>: " + countryNames.join(', ') +
                            " <?php echo e(__('can see and apply to this job.')); ?>";
                    } else {
                        previewText = "<?php echo e(__('Please select at least one country to allow.')); ?>";
                    }
                } else if (restrictionType === 'exclude') {
                    const selectedCountries = $('#excluded_countries option:selected');
                    if (selectedCountries.length > 0) {
                        const countryNames = selectedCountries.map(function() {
                            return $(this).text();
                        }).get();
                        previewText = "<?php echo e(__('Freelancers from')); ?>: " + countryNames.join(', ') +
                            " <?php echo e(__('cannot see or apply to this job.')); ?>";
                    } else {
                        previewText =
                            "<?php echo e(__('This job is available to freelancers from all countries.')); ?>";
                    }
                }

                if (previewText) {
                    $('#country_preview_text').text(previewText);
                    $('#country_preview').show();
                } else {
                    $('#country_preview').hide();
                }
            }

            // Initialize country preview on page load
            updateCountryPreview();
            ...
            $(document).on('change','#category', function() {
                ...
                $('.get_subcategory').html('').trigger('change');
                $.ajax({
                    method: 'post',
        ...
        // Real-time validation on budget input
        $(document).on('input', '#budget', function(){
            let value = parseFloat($(this).val());
            if(value < 0) {
                $(this).val(0);
                toastr_warning_js("<?php echo e(__('Budget cannot be negative')); ?>");
            }
            if(value > 100000) {
                $(this).val(100000);
                toastr_warning_js("<?php echo e(__('Budget cannot exceed 100,000')); ?>");
            }
        });

        //Real-time validation on hourly rate and estimated hours
        $(document).on('input', '#hourly_rate, #estimated_hours', function(){
            let value = parseFloat($(this).val());
            if(value < 0) {
                $(this).val(1);
                toastr_warning_js("<?php echo e(__('Value cannot be negative')); ?>");
            }
            if(value > 100000) {
                $(this).val(100000);
                toastr_warning_js("<?php echo e(__('Value cannot exceed 100,000')); ?>");
            }
        });
    ...
    function pre_next()
    {
        ...
        $(document).on("click", "#next", function (e){
            ...
            if (current <= Listings.length) {
                ...
                // add introduction
                // **MODIFIED: add introduction with restricted word check**
                if(current == 1){
...
<?php /**PATH /Users/ahmetsahin/Desktop/xilancer/core/resources/views/frontend/user/client/job/create/create-job-js.blade.php ENDPATH**/ ?>