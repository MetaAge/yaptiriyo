    ...
    (function($){
        ...
        $(document).ready(function(){
            ...
            function updateCountryPreview() {
                const type = $('#country_restriction_type').val();
                let previewText = '';

                if (type === 'none') {
                    previewText = "<?php echo e(__('This job is available to freelancers from all countries globally.')); ?>";
                } else if (type === 'include') {
                    const selected = $('#allowed_countries option:selected').map(function() {
                        return $(this).text();
                    }).get();
                    previewText = selected.length
                        ? "<?php echo e(__('Only freelancers from')); ?>: " + selected.join(', ') + " <?php echo e(__('can see and apply to this job.')); ?>"
                        : "<?php echo e(__('Please select at least one country to allow.')); ?>";
                } else if (type === 'exclude') {
                    const selected = $('#excluded_countries option:selected').map(function() {
                        return $(this).text();
                    }).get();
                    previewText = selected.length
                        ? "<?php echo e(__('Freelancers from')); ?>: " + selected.join(', ') + " <?php echo e(__('cannot see or apply to this job.')); ?>"
            ...
            // Init on page load (with preselected values)
            updateCountryPreview();
    ...
    function pre_next()
    {
        ...
        $(document).on("click", "#next", function (e){
            ...
            if (current <= Listings.length) {
                ...
                if(current == 1){
                    ...
                    // **ADDED: Check for restricted words before proceeding**
                    ...
                    $('.setup-footer-right').html('<button type="submit" class="btn-profile btn-bg-1" id="confirm_create_job"><?php echo e(__('Edit Job')); ?><span id="job_create_load_spinner"></span></button>');
                }
            }
        ...
        $(document).on("click", "#previous", function (){
            if (current > 0) {
                current--
                if(current == 2){
                    $('.setup-footer-right').html('<input type="submit" class="btn-profile btn-bg-1" value="<?php echo e(__('Update Job')); ?>">');
                }else{
...
<?php /**PATH /Users/ahmetsahin/Desktop/xilancer/core/resources/views/frontend/user/client/job/edit/edit-job-js.blade.php ENDPATH**/ ?>